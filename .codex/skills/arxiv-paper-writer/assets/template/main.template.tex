...
% Marks the first bibliography page so compile_paper.py --report-page-counts
% can separate main-text pages from reference pages. Do not remove.
...
% Define custom accent colors for visualizations
% Use as low-saturation fills (e.g., accentblue!15) per the visual guidance
\definecolor{accentblue}{RGB}{70,130,180}     % Blue for primary elements
\definecolor{accentred}{RGB}{220,20,60}       % Red for contrast or alerts
\definecolor{accentgreen}{RGB}{50,150,50}     % Green for secondary elements
\definecolor{accentpurple}{RGB}{128,0,128}    % Purple for tertiary elements
\definecolor{accentorange}{RGB}{255,140,0}    % Orange for highlighting
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
[Your abstract should be a single paragraph (150-250 words) that summarizes the review. It should clearly state:
(1) the topic and why it matters now,
(2) the scope and how the literature is organized,
(3) the key findings or comparisons the review surfaces, and
(4) open challenges and takeaways.
Be specific about coverage and time window. Avoid citations in the abstract.]
\end{abstract}
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        % Example:
        \node[box=accentblue] (node1) {Input};
        \node[box=accentgreen, below=of node1] (node2) {Process};
        \node[box=accentpurple, below=of node2] (node3) {Output};
